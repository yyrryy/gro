...
def printbarcodewdlink(request):
    products=json.loads(request.GET.get('products'))
    supplierid=request.GET.get('supplierid')
    suppliercode=Supplier.objects.get(pk=supplierid).code
    date=request.GET.get('date')
    ...
    return render(request, 'barcodewdlink.html', {
        'barcodes': barcodes,
    })
...
def getbonsortievalide(request):
...
def getbonsortienonpaye(request):
...
def getbonsortienonvalide(request):
    ...
    if bons:
        ctx['total']=round(Bonsortie.objects.filter(generated=False).order_by('-bon_no').aggregate(Sum('total')).get('total__sum'), 2)
    return JsonResponse(ctx)
